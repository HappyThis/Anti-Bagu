  ...
  return (
    <main className="auth-page">
      <section className="auth-brand-panel auth-brand-panel--register">
        ...
        <div>
          <span>受邀注册</span>
          <h1>使用邀请码，创建你的面试助手账号。</h1>
          <p>邀请码使用一次后自动失效，你的面试记录只属于当前账号。</p>
          <div className="auth-benefits">
            <span><Key size={20} weight="duotone" /><b>一次性邀请码注册</b></span>
      ...
      <section className="auth-form-panel">
        <form className="auth-form" onSubmit={submit}>
          ...
          <h2>创建账户</h2>
          <p>填写管理员提供的一次性邀请码。</p>
          <label><span>邀请码</span><input required autoComplete="one-time-code" placeholder="AB-XXXX-XXXX-XXXX" value={activationKey} onChange={(event) => setActivationKey(event.target.value.toUpperCase())} /></label>
          ...
          <label><span>再次输入密码</span><input required autoComplete="new-password" minLength={8} type="password" placeholder="再次输入相同密码" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} /></label>
          {confirmPassword ? <div className={password === confirmPassword ? 'password-match password-match--ok' : 'password-match password-match--error'}>{password === confirmPassword ? '两次密码一致' : '两次密码不一致'}</div> : null}
          {error ? <div className="form-error" role="alert">{error}</div> : null}
          <button className="primary-action" type="submit" disabled={loading || !confirmPassword || password !== confirmPassword}>
            {loading ? '正在注册并登录…' : '验证并进入'}<ArrowRight size={19} />
          </button>
