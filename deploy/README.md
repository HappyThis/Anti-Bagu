...
1. Install Node.js, Swift, and rsync on the developer Mac; install Python,
   Nginx, PostgreSQL, and Certbot 5.4+ on the server.
2. Create the `antibagu` system user and PostgreSQL database.
3. Build the web application and macOS Agent locally, then synchronize the
   release to `/opt/anti-bagu`.
4. Create the server Python environment and run `alembic upgrade head`.
